...
Implemented in the current repository: a trusted local Task Contract registry that resolves `contractId` before Cedar/AVP authorization, backend-normalized tool/action/resource/argument metadata, immutable hash-covered execution evidence for the current filesystem path, separate archival receipts linked to primary event hashes, safe provenance-source hashing/redaction, bounded session reconstruction, structured Bedrock investigation results with event/hash references, bearer-token API access protection for sensitive backend routes via `AEGIS_API_TOKEN`, Docker packaging for the current Express backend, a minimal ECS/Fargate + ALB deployment artifact with desired count `1`, and a static executor registry containing only the current `fs:read` filesystem boundary. `AEGIS_API_TOKEN` is an API access-control boundary for the local/demo deployment. It is not cryptographic authentication of an AI agent, and a token exposed to a browser client must not be treated as confidential. When supplied to a browser client through Vite environment configuration as `VITE_AEGIS_API_TOKEN`, the token is necessarily observable by that client and therefore is not a confidential credential or cryptographic agent identity. Not implemented in the current repository: API Gateway, Lambda, KMS, cryptographic signed Task Contract verification, HMAC/session tokens, npm/git/shell/network/MCP execution, durable reconstruction from DynamoDB/S3, and EventBridge consumers. Bedrock remains post-hoc only; live invocation requires an active configured model or inference profile and account model access.
...
1. **Amazon API Gateway & Lambda:** Target topology only. The current repository implements the PEP as an Express server and Phase 8 packages it for a single-task ECS/Fargate + ALB deployment proof.
...
Phase 8 deployment boundary: the filesystem executor operates on repository artifacts packaged into the Aegis container. ECS/Fargate hosting does not by itself prove general sandbox, network isolation, cryptographic agent identity, or durable evidence reconstruction. The Phase 8 template intentionally uses one desired task because the ledger is process-local.
