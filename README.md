...
- Protected backend routes require `Authorization: Bearer <AEGIS_API_TOKEN>` before normalization, authorization, execution, evidence reads, policy-source reads, reconstruction, or investigation. `/api/health`, `/api/aegis/status`, and `/api/aegis/capabilities` remain public truth/status endpoints. This is an API access boundary, not cryptographic agent authentication.
